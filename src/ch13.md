...
- Jednotlivé části kódu organizujeme do tzv. **komponent**, které zodpovídají za jednotlivé části stromu dokumentu. V&nbsp;jazyce JSX komponenty vypadají jako HTML značky a&nbsp;nápadně připomínají *Custom Elements* (devátá kapitola). Komponenty jsou typicky implementovány jako JavaScriptové funkce.
...
Nenechme se zastrašit větším množstvím nezvyklých konstrukcí a&nbsp;pojďme si je po částech rozebrat. Poslední část funkce `App` definuje výsledné HTML, resp. výsledné JSX. To odpovídá přístupu **komponenty jsou funkce, které transformují vstup na HTML**. Každá komponenta musí vrátit jen jeden prvek, takže když chceme vykreslit více HTML (nadpis, formulář, výsledky), musíme je zabalit do společného *bezejmenného* obalu, který se v&nbsp;JSX zapisuje jako `<>...</>`.
