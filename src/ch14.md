...
Abychom tento objekt (když má metodu `next()`, říkáme mu *iterátor*) mohli při iteraci použít, zbývá ho vložit na správné předem dohodnuté místo. Konkrétně jej musí *vrátit funkce, kterou do iterovaného datového typu vložíme pod klíč `Symbol.iterator`*. Zní to složitě, ale implementace je jednoduchá. Můžeme si pomoci například třídou:
