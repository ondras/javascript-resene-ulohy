...
1. **Funkce volaná jako metoda objektu**, tj. vlevo od jejího názvu je objekt oddělený tečkou: hodnotou `this` bude objekt vlevo od tečky. To odpovídá přístupu v jiných objektově orientovaných jazycích.

1. **Funkce předaná jako callback**, tj. vykonaná v rámci jiné funkce.
...
1. **Speciální případy**:
    ...
    - V ostatních případech je hodnota `this` buď nedefinována, nebo je to globální objekt (v prohlížeči `window`). Pravidla pro tuto situaci jsou komplikovaná, ale nemusíme jim rozumět --
