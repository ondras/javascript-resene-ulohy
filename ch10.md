...
Výše uvedený polyfill nelze považovat za plnohodnotnou náhradu objektu `DateTimeFormat`. Jednak nerespektuje zadaný jazyk, jednak vůbec není možné konfigurovat formát výpisu pomocí druhého parametru konstruktoru. Zvídavý čtenář může ale snadno metodu `format` obohatit i o další formáty data či  času.
...
1. Když se jedná o změnu *syntaxe* jazyka (příklad: klíčová slova `async/await`, arrow funkce, `class` a další). Kód našeho polyfillu nedovede naučit parser JavaScriptu novým pravidlům; můžeme jím jen přidávat nové vlastnosti a funkce existujícím rozhraním.
...
- **Web MIDI**: rozhraní pro spolupráci s externími zařízeními, které generují či přijímají tzv. *MIDI události*, používané zejména v hudební produkci.
...
Z výčtu jsou záměrně vypuštěna tři rozhraní, určená pro práci s grafikou (SVG, Canvas a WebGL). To proto, že jim bude věnována celá následující kapitola.
