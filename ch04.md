...
function checkEmpty(input) {
	if (input.value.trim() == "") { throw new Error("Pole nesmí být prázdné"); }
}
...
```js
const DEBOUNCE = 500;
let timeout;  // identifikace prave naplanovane kontroly
