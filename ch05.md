...
(Termíny *pravdivá* resp. *nepravdivá* odpovídají intiutivnímu procesu převodu libovolné hodnoty na boolean. Tušíme, že `false` bude nula, prázdný řetězec a podobně. Vše ostatní je `true`. Přesněji se na tyto převody podíváme hned v následující podkapitole pro koumáky.)
...
Pro operátor `||` teď žádné vhodné využití nemáme, ale dostaneme se k němu v kapitole FIXME.
...
Pokud nás tato flexibilita trápí, můžeme šáhnout po jednom ze dvou řešení:

1. Namísto operátoru rovnosti (`==`) používat *trojrovnítkový operátor ekvivalence* (`===`). Ten funguje obdobně jako běžné porovnání, ale pokud jeho operandy mají různý typ, ihned vrátí `false`. Nejprojde skrz něj tedy žádný ze tří testů uvedených v předchozí ukázce.
